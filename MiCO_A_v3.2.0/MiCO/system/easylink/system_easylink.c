...
OSStatus system_easylink_start( system_context_t * const inContext )
{
    ...
    err = mico_rtos_create_thread( &easylink_thread_handler, MICO_APPLICATION_PRIORITY, "EASYLINK",
                                   easylink_thread, 0x1000, (mico_thread_arg_t) inContext );
...
void easylink_thread( uint32_t inContext )
{
    ...
    if ( easylink_success == true )
    {
        mico_system_delegate_config_recv_ssid(
            Context->flashContentInRam.micoSystemConfig.ssid,
        ...
        require_noerr_action_string( err, restart, micoWlanSuspend(),
                                     "Re-start easylink commbo mode" );
        mico_system_delegate_config_success( source );
        if ( source == CONFIG_BY_AIRKISS )
        {
            err = mico_rtos_create_thread( NULL, MICO_APPLICATION_PRIORITY, "AIRKISS",
                                           airkiss_broadcast_thread, 0x800, 0 );
            require_noerr_string( err, exit, "ERROR: Unable to start the EasyLink thread." );
        }
        goto exit;
    }
