...
typedef enum
{
    ...
    eState_HandleConfigMessageFIN       = 5,
...
char *destinationPSK  = 0;
char *accessoryName   = 0;
char *playPassword    = 0;

extern WACPlatformParameters_t* WAC_Params;

static mico_semaphore_t close_server_sem = 0;
...
extern mico_i2c_t _mfi_wac_i2c( void );
static OSStatus CreateTLVConfigResponseMessage( uint8_t **outTLVResponse, unsigned int *outTLVResponseLen);
static void http_server_thread( mico_thread_arg_t inContext );
static OSStatus _ParseTLVConfigMessage( const void * const inTLVPtr,
                                        const unsigned int inTLVLen,
                                        char **outSSID,
                                        char **outPSK,
                                        char **outName,
                                        char **outPlayPassword )
{
    ...
    do { if( ! !(!(inTLVPtr)) ) { ; goto exit; } } while( 1==0 );
    do { if( ! !(!(inTLVLen)) ) { ; goto exit; } } while( 1==0 );
    do { if( ! !(!(outSSID)) ) { ; goto exit; } } while( 1==0 );
    do { if( ! !(!(outPSK)) ) { ; goto exit; } } while( 1==0 );
    do { if( ! !(!(outName)) ) { ; goto exit; } } while( 1==0 );
    do { if( ! !(!(outPlayPassword)) ) { ; goto exit; } } while( 1==0 );
...
static void http_server_thread( mico_thread_arg_t inContext)
{
  int i, len, fd_listen = -1, err = 0;
  struct sockaddr_in addr;
  fd_set readfds;
  struct timeval t;
  char ip_address[16];
  mico_Context_t *Context = (mico_Context_t *)inContext;
  int close_server_event_fd = -1;

  HTTPHeader_t *httpHeader = 0;

  mico_rtos_init_semaphore( &close_server_sem, 1);
  close_server_event_fd = mico_create_event_fd( close_server_sem );
  
  httpHeader = HTTPHeaderCreate( 128 );
  ...
  if ( fd_listen == -1 ) {
    fd_listen = socket( 2, 1, 6 );

    addr.sin_family = 2;
    addr.sin_addr.s_addr =((uint32_t)0x00000000UL);
    addr.sin_port = ( (uint16_t)( ( ( ( (uint16_t)(_port) ) << 8 ) & 0xFF00 ) | ( ( ( (uint16_t)(_port) ) >> 8 ) & 0x00FF ) ) );
  ...
  while(1){
    ...
    ((&readfds)->fds_bits[(fd_listen)/(sizeof(unsigned long) * 8)] |= ((unsigned long)1 << ((fd_listen) % (sizeof(unsigned long) * 8))));  
    ((&readfds)->fds_bits[(close_server_event_fd)/(sizeof(unsigned long) * 8)] |= ((unsigned long)1 << ((close_server_event_fd) % (sizeof(unsigned long) * 8))));
    if (connected_socket != -1)
      ((&readfds)->fds_bits[(connected_socket)/(sizeof(unsigned long) * 8)] |= ((unsigned long)1 << ((connected_socket) % (sizeof(unsigned long) * 8))));
    
    select(1, &readfds, 0, 0, &t);

     
    if(((&readfds)->fds_bits[(close_server_event_fd)/(sizeof(unsigned long) * 8)] & ((unsigned long)1 << ((close_server_event_fd) % (sizeof(unsigned long) * 8))))){
      mico_rtos_get_semaphore( &close_server_sem, 0 );
      goto exit;
    }
    
     
    if(((&readfds)->fds_bits[(fd_listen)/(sizeof(unsigned long) * 8)] & ((unsigned long)1 << ((fd_listen) % (sizeof(unsigned long) * 8))))){
      i = accept(fd_listen, (struct sockaddr *)&addr, (uint32_t *)&len);
      if (i >= 0) {
        strcpy(ip_address,inet_ntoa( addr.sin_addr ));
        ;
        SocketClose( &connected_socket );
        connected_socket = i;
    ...
    if (((&readfds)->fds_bits[(connected_socket)/(sizeof(unsigned long) * 8)] & ((unsigned long)1 << ((connected_socket) % (sizeof(unsigned long) * 8))))) {
        ...
        switch ( err )
        {
          case 0:
              ...
              do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; goto client_close; } } while( 1==0 );
...
client_close:
        SocketClose( &connected_socket );
        HTTPHeaderClear( httpHeader );
        if( client_state == eState_HandleConfigMessageFIN ){
          ...
          err = 0;         
        }else if( client_state == eState_Complete ){
          client_state = eState_Cleanup;
          mfi_wac_stop();
        }else{
...
exit:
  ...
  if( close_server_sem != 0 ){
    mico_delete_event_fd( close_server_event_fd );
    mico_rtos_deinit_semaphore( &close_server_sem );
    close_server_sem = 0;
  };
  ...
  if(mfiSAPRef) {
    MFiSAP_Delete( mfiSAPRef );
  }

  SocketClose( &fd_listen );
  SocketClose( &connected_socket );
...
static OSStatus _HandleState_WaitingForAuthSetupMessage( HTTPHeader_t* inHeader, _WACState_t *inState, mico_Context_t * const inContext )
{
  ...
  err = HTTPHeaderMatchURL( inHeader, "/auth-setup" );
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; { ; err = -6769; } goto exit; } } while( 1==0 );
...
static OSStatus _HandleState_HandleAuthSetupMessage( HTTPHeader_t* inHeader, _WACState_t *inState, mico_Context_t * const inContext )
{
  ...
  uint8_t *mfiSAPResponseDataPtr = 0;
  unsigned int mfiSAPResponseDataLen = 0;

  err =  MicoMFiAuthInitialize( _mfi_wac_i2c() );
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; goto exit; } } while( 1==0 );
  
  if(mfiSAPRef) {
    MFiSAP_Delete( mfiSAPRef );
  }
  err = MFiSAP_Create( &mfiSAPRef, 1 );
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; goto exit; } } while( 1==0 );

  
  err = MFiSAP_Exchange( mfiSAPRef,                            
                         ...
                         &mfiSAPComplete );                               
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; { ; } goto exit; } } while( 1==0 );
  do { if( ! !(!(mfiSAPComplete)) ) { ; goto exit; } } while( 1==0 );

  err =  CreateSimpleHTTPMessage( "application/octet-stream", mfiSAPResponseDataPtr, mfiSAPResponseDataLen, &httpResponse, &httpResponseLen );
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; goto exit; } } while( 1==0 );
  do { if( ! !(!(httpResponse)) ) { ; goto exit; } } while( 1==0 );

  sentNum = send( connected_socket, httpResponse, httpResponseLen,0 );
  do { if( ! !(!(sentNum==(int)httpResponseLen)) ) { ; goto exit; } } while( 1==0 );
  ;
...
static OSStatus _HandleState_WaitingForConfigMessage( HTTPHeader_t* inHeader, _WACState_t *inState, mico_Context_t * const inContext )
{
  ...
  err = HTTPHeaderMatchURL( inHeader, "/config" );
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; { ; err = -6769; } goto exit; } } while( 1==0 );
...
static OSStatus _HandleState_HandleConfigMessage( HTTPHeader_t* inHeader, _WACState_t *inState, mico_Context_t * const inContext )
{
  ...
  do { if( ! !(!(decryptedConfigData)) ) { ; { err = -6728; } goto exit; } } while( 1==0 );
  err = MFiSAP_Decrypt( mfiSAPRef,                 
                        inHeader->extraDataPtr,  
                        inHeader->extraDataLen,  
                        decryptedConfigData );                
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; goto exit; } } while( 1==0 );

  
  err = _ParseTLVConfigMessage( decryptedConfigData,                  
                                inHeader->extraDataLen,               
                                &destinationSSID,                     
                                &destinationPSK,                      
                                &accessoryName,                       
  ...
  do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; { err = -6712; } goto exit; } } while( 1==0 );
  do { if( ! !(!(destinationSSID)) ) { ; { err = -6712; } goto exit; } } while( 1==0 );
  ...
  mico_system_delegate_config_recv_ssid ( destinationSSID, destinationPSK );
  ...
  if ( App_Available )
  {
      ...
      unsigned int  eaTLVResponseLen;
      err = CreateTLVConfigResponseMessage( &eaTLVResponse, &eaTLVResponseLen );
      do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; goto exit; } } while( 1==0 );
      do { if( ! !(!(eaTLVResponse)) ) { ; { err = -6728; } goto exit; } } while( 1==0 );
      do { if( ! !(!(eaTLVResponseLen)) ) { ; { err = -6728; } goto exit; } } while( 1==0 );

      
      uint8_t *encryptedConfigData = malloc( eaTLVResponseLen * sizeof( uint8_t ) );
      do { if( ! !(!(encryptedConfigData)) ) { ; goto exit; } } while( 1==0 );

      err = MFiSAP_Encrypt( mfiSAPRef,     
                            eaTLVResponse,            
                            eaTLVResponseLen,         
                            encryptedConfigData );    

      err = CreateSimpleHTTPMessage( "application/x-tlv8",      
                                     encryptedConfigData, 
                                     eaTLVResponseLen,    
                                     &httpResponse,       
                                     &httpResponseLen );  
  ...
  *inState = eState_HandleConfigMessageFIN;
...
static OSStatus _HandleState_WaitingForConfiguredMessage( HTTPHeader_t* inHeader, _WACState_t *inState, mico_Context_t * const inContext )
{
    ...
    err = HTTPHeaderMatchURL( inHeader, "/configured" );
    do { OSStatus localErr; localErr = (OSStatus)(err); if( ! !(localErr != 0) ) { ; { ; err = -6769; } goto exit; } } while( 1==0 );
...
static OSStatus CreateTLVConfigResponseMessage( uint8_t **outTLVResponse, unsigned int *outTLVResponseLen)
{
    ...
    *outTLVResponseLen = 0;
    ...
    if ( WAC_Params->name )
    {
        nameSize = strnlen( WAC_Params->name, 255 );
        *outTLVResponseLen += nameSize + 2;
    }

    if ( WAC_Params->manufacturer )
    {
        manufacturerSize = strnlen( WAC_Params->manufacturer, 255 );
        *outTLVResponseLen += manufacturerSize + 2;
    }

    if ( WAC_Params->model )
    {
        modelSize = strnlen( WAC_Params->model, 255 );
        *outTLVResponseLen += modelSize + 2;
    }

    if ( WAC_Params->firmwareRevision )
    {
        firmwareRevisionSize = strnlen( WAC_Params->firmwareRevision, 255 );
        *outTLVResponseLen += firmwareRevisionSize + 2;
    }

    if ( WAC_Params->hardwareRevision )
    {
        hardwareRevisionSize = strnlen( WAC_Params->hardwareRevision, 255 );
        *outTLVResponseLen += hardwareRevisionSize + 2;
    }

    if ( WAC_Params->serialNumber )
    {
        serialNumberSize = strnlen( WAC_Params->serialNumber, 255 );
        *outTLVResponseLen += serialNumberSize + 2;
    }

    if ( WAC_Params->eaBundleSeedID )
    {
        eaBundleSeedIDSize = strnlen( WAC_Params->eaBundleSeedID, 255 );
        *outTLVResponseLen += eaBundleSeedIDSize + 2;
    }

    eaProtocolSizes = calloc( WAC_Params->numEAProtocols, sizeof( uint32_t ) );
    do { if( ! !(!(eaProtocolSizes)) ) { ; { err = -6728; } goto exit; } } while( 1==0 );

    uint8_t protocolIndex;
    for( protocolIndex = 0; protocolIndex < WAC_Params->numEAProtocols; protocolIndex++ )
    {
        eaProtocolSizes[protocolIndex] = strnlen( WAC_Params->eaProtocols[protocolIndex], 255 );
        *outTLVResponseLen += eaProtocolSizes[protocolIndex] + 2;
    }
    ...
    *outTLVResponse = calloc( *outTLVResponseLen, sizeof( uint8_t ) );
    ...
    do { if( ! !(!(( tlvPtr - *outTLVResponseLen ) == *outTLVResponse)) ) { ; { err = -6743; } goto exit; } } while( 1==0 );
...
OSStatus mfi_wac_http_server_start(void *inContext)
{
  return  mico_rtos_create_thread(0, (7), "MFI_TCP_SERVER", http_server_thread, 0x1D00, (mico_thread_arg_t)inContext );
}

OSStatus mfi_wac_http_server_stop( void )
{
  if( close_server_sem != 0 )
    mico_rtos_set_semaphore( &close_server_sem );
  return 0;
}
